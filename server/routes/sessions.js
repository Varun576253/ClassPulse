...
router.post('/:sessionId/submit', async (req, res) => {
  try {
    const { studentMobile, answers = {} } = req.body;
    ...
    let student = await Student.findOne({ teacherId: session.teacherId, phone: `91${cleanMobile}` });
    if (!student) {
      return res.status(403).json({
        success: false,
        error: 'You are not registered in this class.'
      });
    }
...
router.post('/:sessionId/student-login', async (req, res) => {
  try {
    ...
    if (session.status !== 'active' || session.formStatus === 'closed') {
      return res.status(400).json({ success: false, error: 'This session is not currently accepting responses.' });
    }

    const student = await Student.findOne({ teacherId: session.teacherId, phone: `91${cleanMobile}` });
    if (!student) {
      return res.status(403).json({
        success: false,
        error: 'Your mobile number is not registered in this class. Please ask your teacher to add you to the roster first.'
      });
    }

    const alreadySubmitted = (session.responses || []).some(r => r.studentMobile === cleanMobile);
    if (alreadySubmitted) {
      return res.status(400).json({ success: false, error: 'You have already submitted this quiz.' });
    }
