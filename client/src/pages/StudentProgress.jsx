...
const StudentProgress = () => {
  ...
  const topicMastery = useMemo(() => {
    const map = {};
    (student?.progressHistory || []).forEach((item) => {
      if (!item.topic) return;
      const existing = map[item.topic];
  ...
  const gapTrendSessions = useMemo(() => (student?.progressHistory || []).map((item) => ({
    topic: item.topic,
    date: item.date,
    classInsight: {
      understoodCount: item.understood ? 1 : 0,
      partialCount: (!item.understood && Number(item.score || 0) >= 50) ? 1 : 0,
      strugglingCount: (!item.understood && Number(item.score || 0) < 50) ? 1 : 0
    },
    responses: [{}]
  })), [student]);
  ...
  return (
    <div className="space-y-5">
      ...
      {topicMastery.length > 0 && (
        <section className="panel rounded-xl p-5">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-wider text-slate-400">Concept by concept</p>
              <h2 className="mt-1 text-lg font-black text-[#11233f]">Topic mastery tracker</h2>
            </div>
            <span className="text-xs font-semibold text-slate-400">{topicMastery.length} topics assessed</span>
          </div>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {topicMastery.map((item) => {
              const score = Number(item.score || 0);
              const mastered = item.understood && score >= 75;
              const partial = !mastered && score >= 50;
              const bg = mastered ? 'bg-emerald-50 border-emerald-200' : partial ? 'bg-amber-50 border-amber-200' : 'bg-rose-50 border-rose-200';
              const badge = mastered ? 'text-emerald-600' : partial ? 'text-amber-600' : 'text-rose-600';
              const label = mastered ? 'Mastered' : partial ? 'Partial' : 'Gap';
              return (
                <div key={item.topic} className={`rounded-xl border p-3 ${bg}`}>
                  <p className="text-xs font-black text-[#11233f] truncate mb-1" title={item.topic}>{item.topic}</p>
                  <div className="flex items-center justify-between gap-2">
                    <span className={`text-[10px] font-black uppercase ${badge}`}>{label}</span>
                    <span className={`text-sm font-black ${badge}`}>{score}%</span>
                  </div>
                  <p className="mt-1 text-[10px] text-slate-400">{new Date(item.date).toLocaleDateString([], { day: '2-digit', month: 'short' })}</p>
                </div>
              );
            })}
          </div>
      ...
      {gapTrendSessions.length >= 2 && (
        <LearningGapTrend sessions={gapTrendSessions} />
      )}
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-slate-500">
                      {analysis.assessment?.subject || student.teacherId?.subject} · {new Date(analysis.createdAt).toLocaleDateString()}
                    </p>
                  </div>
              ...
              <div className="space-y-2">
                {remediationHistory.slice(0, 4).map((item) => (
                  <div key={item._id} className="rounded-lg bg-indigo-50 px-3 py-2">
                    ...
                    <p className="mt-1 text-xs text-slate-500">{item.materialType} · {item.assessment?.topic || 'Assessment'}</p>
                  </div>
                ))}
              </div>
            </div>
